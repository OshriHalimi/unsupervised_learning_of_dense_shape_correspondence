...
flags.DEFINE_string('test_shapes_dir', './tf_artist/', '')
...
def get_test_list():
	...
	#f.close()
	test_pairs = [['model_0.mat','model_1.mat']]
	return test_pairs
...
def run_test():
	...
	saver = tf.train.import_meta_graph('%s/model.ckpt-600.meta'%FLAGS.log_dir)
	...
	for test_pair in test_list:
		...
		sio.savemat('./Results/unsupervised_artist_results/' + '{}_{}.mat'.format(test_pair[0][:-4], test_pair[1][0:-4]), params_to_save)
